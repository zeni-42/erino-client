...
const formDataSchema = z.object({
    ...
    source: z.enum(["website", "facebook_ads", "google_ads", "referral", "events", "other"]).optional(),
...
type EditFromValues = z.infer<typeof formDataSchema>
...
export default function EditForm({ onClose, leadId }: LeadFormInterface ){
    const [isLoading, setIsLoading] = useState(false)

    const { register, handleSubmit, reset, formState: { errors }, control } = useForm<EditFromValues>({
        resolver: zodResolver(formDataSchema),
        mode: "onChange",
    })

    const handleUpdateLead: SubmitHandler<EditFromValues> = async (data: EditFromValues) => {
        setIsLoading(true)
        try {
            const res = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/leads/${leadId}`, data, { withCredentials: true })
            if (res.status == 201) {
        ...
        <div className="fixed top-0 left-64 w-[calc(100vw-16rem)] h-screen z-10 bg-white p-7 overflow-y-hidden" >
            <div className="flex justify-between items-center">
                <div className="flex justify-center items-start flex-col space-y-1" >
                    <h1 className="text-2xl font-medium text-zinc-700">Update lead for id: {leadId}</h1>
                    <p className="text-sm text-zinc-600">Update lead data properly</p>
                </div>
            ...
            <div className="w-full mt-10" >
                <form onSubmit={handleSubmit(handleUpdateLead)} className="w-full px-10 " >
